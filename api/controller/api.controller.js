...
let getDeviceList = (req, res) =>  {
    ...
    const queryDB = "SELECT device_id, device_name FROM device WHERE room_id = ?";
    //console.log(queryDB)
                        ...
                        if(fulfilled === true){
                            db.getConnection((err, conn) => {
                                if (err) {
                                    ...
                                    conn.query(queryDB, [room_id], (err, data) =>  {
                                        //console.log(data)
                                        conn.release();
                                        if(err){
                                            res.json({
...
let addDevice = (req, res) => {
    ...
    console.log(home_id)
    let _query = "INSERT INTO device (room_id, device_name) VALUES (?,?)";
                       ...
                       if(__result === true){
                           db.getConnection((err, conn) => {
                               if(err){
                                   ...
                                   conn.query(_query, [parseInt(room_id), device_name], err => {
                                       //console.log(conn)
                                       conn.release();
                                       //console.log(err)
                                       if(err){
                                           if (err.errno === 1062) {
                                               res.json({
                                                   success: false,
                                                   reason: device_name + " has already existed in room " + room_id
                                               })
                                           } else {
                                               res.json({
                                                   success: false,
                                           ...
                                           res.json({
                                              success: true,
                                              reason: "Success in adding device " + device_name + " into room " + room_id
                                           });
                                       }
                                   })
                               }
...
let delete_device = (req, res) => {
    ...
    const queryDB = "DELETE FROM device WHERE device_name = ?";
                        ...
                        if(__result === true){
                            db.getConnection((err, conn) => {
                                if(err){
                                    ...
                                    conn.query(queryDB, [device_name], err => {
                                        conn.release();
                                        console.log(err)
                                        if(err){
                                            res.json({
                                            ...
                                            let message = "Successfully in deleting device " + device_name;
                                            res.json({
                                                success: true,
                                                message: message
                                            });
