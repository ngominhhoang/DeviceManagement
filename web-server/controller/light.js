...
let getDeviceList = (req, res) => {
    ...
    const options = {
        url: process.env.SERVER_ADD + '/api/data/home/' + home_id + '/room/' + room_id  + '?token=' + token,
        method: 'GET'
    };
...
let addDevice = (req, res) => {
    ...
    let device_name = req.body.getNewDeviceName;
...
let calculatePatternByGA = (data, home_id, room_id, token, res) => {
    console.log('aaa');
    const options = {
    ...
    requestLightLevel.then(result => {
        requestIlluminanceSensor.then(_result => {
            requestBasicLighting.then(__result => {
                ...
                //hardcode
                basicLighting = [20,0,0,0,0,0,0,20,0,0,0,0,0,0,20,0,0,0,0,0,0,20,0,0,0,0,0,0,20,0,0,0,0,0,0,20]
...
let turnOnLight = (req, res) => {
    ...
    console.log(data);
    //Xử lí mảng ở đây
    calculatePatternByGA(data, home_id, room_id, token, res);
    for (let i=0; i<watcherList.length; ++i) {
        if (watcherList[i]['room'] === room_id) {
            clearInterval(watcherList[i]['adapter'])
    ...
    let watcher = {
        room_id: room_id,
        adapter: setInterval(() => {
            calculatePatternByGA(data, home_id, room_id, token, null);
        }, 5000)
    }
